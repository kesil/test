...
const G = {
  ...
  score: 0, deaths: 0, timeF: 0, totalScore: 0, totalDeaths: 0,
  narrFired: {}, toast: null, dialog: null, gag: null, chaser: null, boss: null,
  checkpoint: null, gsign: 1, freeze: 0, fade: 0, fadeDir: 0,
...
function tileAt(px, py) {
  ...
  if (tx < 0 || tx >= L.w || ty < 0) return T.BRICK;   // sides/top solid-ish
  if (ty >= L.h) return T.NONE;                        // below map = pit
...
function toast(msg, time = 210) { G.toast = { msg, t: time }; Audio2.sfx('ui'); }
...
// flip gravity, re-anchoring the player's feet so he isn't embedded in terrain
...
// entity solids (paywall) applied to player collision
function paywallBox(e) { return { x: e.x - 16, y: e.y - 64, w: 32, h: 64 }; }
function extraSolids() {
  const out = [];
  for (const e of G.ents) if (e.type === 'paywall' && !e.dead) out.push(paywallBox(e));
  return out;
}

// ---------- physics ----------
function moveBody(b, solidsExtra) {
  // b: {x (center), y (feet), w, h, vx, vy, onGround...} respects G.gsign for player
...
function makePlayer(x, y) {
  ...
  return {
    ...
    inv: 0, star: 0, chonk: 0, umbrella: false, jetpack: -1, fuel: 0, mecha: false,
    coyote: 0, jbuf: 0, rollT: 0, rollCd: 0, digT: 0, swipeT: 0, throwT: 0, shootCd: 0,
    state: 'normal', stateT: 0, idleT: 0, eatT: 0, cart: false, cartV: 0,
    dead: false, wobble: 0, tauntT: 0, climbing: false, swimming: false,
  };
}

function hurtPlayer(dmg, fromX) {
  const p = G.p;
  if (GOD || p.inv > 0 || p.star > 0 || p.dead || G.state !== 'play') return;
...
function respawn() {
  ...
  Object.assign(p, {
    x: cp.x, y: cp.y, vx: 0, vy: 0, hearts: p.maxHearts, dead: false,
    state: 'normal', stateT: 0, inv: 90, star: 0, chonk: 0, rollT: 0,
  });
  if (p.jetpack >= 0) p.fuel = 100;   // never softlock the flight finale
  if (G.cfg.cart) { p.cart = true; p.cartV = 0; }
  if (G.gsign < 0) G.gsign = 1;
  if (G.chaser) G.chaser.x = Math.max(G.chaser.startX, p.x - 260);
  G.cam.x = clamp(p.x - VW / 2, 0, G.level.w * TILE - VW);
}

function updatePlayer(dt) {
  const p = G.p;
  const atlas = Assets.json[p.atlas];
  ...
  if (inWater) {
    ...
    p.anim.set(Input.d ? 'dive' : 'swim');
    p.anim.update(p.anim.name === 'dive' ? 'v6a' : 'v5a', dt);
    p.atlas = p.anim.name === 'dive' ? 'v6a' : 'v5a';
    p.sheet = p.anim.name === 'dive' ? 'v6' : 'v5';
  ...
  if (p.climbing) {
    if (!onLadder) p.climbing = false;
      ...
      if (p.onGround || tileAt(p.x, p.y + 2) === T.NONE && !onLadder) p.climbing = false;
      postMovePlayer();
      return;
    }
  }
  ...
  // ---- horizontal ----
  const spd = 2.15 * (p.chonk ? 0.82 : 1) * (p.star ? 1.15 : 1) * (p.onWire ? 0.72 : 1);
  ...
  if (p.umbrella && Input.jump && p.vy * gs > 0) maxFall = 1.05;
  p.vy += grav * gs;
  ...
  if (p.digT > 0) {
    ...
    if (p.digT === 0 && p.digSpot) {
      ...
      narrOnce('firstDig');
      const roll = Math.random();
      if (roll < 0.5) { addScore(50, p.x, p.y - 40); addFX('v5', 'fx_sparkle', p.x, p.y - 20); }
      else if (roll < 0.75) { spawnItem('can', p.x, p.y - 8); }
      else if (roll < 0.9) { spawnItem('cookie', p.x, p.y - 8); }
  ...
  if (!wasGround && p.onGround) {
    Audio2.sfx('land');
    if (p.vyPrev > 4) addFX('v5', 'fx_dust', p.x, p.y);
  }
  ...
  if (p.mecha) { sheet = 'v6'; anim = 'mecha'; }
...
function postMovePlayer() {
  ...
  // tile interactions across body
  for (const ty of tilesInRange(box.y, box.h)) {
    for (const tx of tilesInRange(box.x, box.w)) {
      const c = tileAt(tx * TILE + 1, ty * TILE + 1);
      if (c === T.CONE || c === T.GLASS) hurtPlayer(1, tx * TILE + 8);
      else if (c === T.FLAG) {
        const key = tx + ',' + ty;
        if (!G.flagsLit[key]) {
          G.flagsLit[key] = true;
...
function useDoor(tx, ty) {
  ...
  p.vx = 0; p.vy = 0;
  Audio2.sfx('power');
  if (!G.discoFound && G.cfg.id === 1 && d.x > 180) {
    G.discoFound = true;
    narrOnce('disco');
...
function swipeHit() {
  ...
  for (const e of G.ents) {
    if (e.dead) continue;
    if (['rat', 'gull', 'drone', 'robot'].includes(e.type) && aabb(hb, entBox(e))) {
  ...
  if (hitAny) addFX('v6', 'fx_hitspark', p.x + p.dir * 20, p.y - 14);
}
...
function updateExit(dt) {
  ...
  G.exitT++;
  if (G.cfg.boss) {
    // boss defeated: celebrate on the spot
    p.vx = 0;
...
function entBox(e) {
  switch (e.type) {
    ...
    case 'gull': return { x: e.x - 12, y: e.y - 14, w: 24, h: 14 };
...
function damageEnemy(e, dmg, fromX) {
  ...
  if (e.hp <= 0) {
    e.dead = true;
    addScore(e.type === 'robot' ? 150 : 100, e.x, e.y - 20);
    addFX('v5', 'fx_dust', e.x, e.y);
    if (e.type === 'robot') {
      spawnItem('can', e.x - 8, e.y - 8); spawnItem('can', e.x + 8, e.y - 8);
      narrOnce('robotDown');
    }
    if (e.type === 'gull') narrOnce('gullDown');
...
function updateEnemies(dt) {
  ...
  for (const e of G.ents) {
    ...
    switch (e.type) {
      ...
      case 'possum': {
        ...
        if (Math.abs(p.x - e.x) < 60 && !e.talked) {
          ...
          const lines = [
            "Randall. Possum. I live in the wall. It's rent-controlled.",
            "I'm not playing dead. This is a lifestyle.",
            "You want the trash? The BIG bin has it. He's got a WHOLE thing.",
          ];
    ...
    if (['rat', 'gull', 'drone', 'robot'].includes(e.type) && !e.dead) {
      ...
      if (aabb(pb, eb)) {
        if (p.star > 0 || p.rollT > 0) damageEnemy(e, p.star ? 9 : 1, p.x);
          ...
          // stomp
          damageEnemy(e, e.type === 'robot' ? 1 : 1, p.x);
          p.vy = -4.5; Audio2.sfx('stomp');
        } else hurtPlayer(e.type === 'robot' ? 2 : 1, e.x);
      }
    }
  }
  // box respawns (boss arena can dispensers)
  ...
  for (const s of G.shots) {
    ...
    switch (s.kind) {
      case 'can':
        ...
        for (const e of G.ents) {
          if (!e.dead && ['rat', 'gull', 'drone', 'robot'].includes(e.type) && aabb({ x: s.x - 5, y: s.y - 5, w: 10, h: 10 }, entBox(e))) {
          ...
          if (e.type === 'paywall' && !e.dead && aabb({ x: s.x - 5, y: s.y - 5, w: 10, h: 10 }, paywallBox(e))) {
            ...
            if (e.hp <= 0) {
              ...
              Audio2.sfx('fanfare');
            } else toast('CANCELING... ' + e.hp + ' can' + (e.hp > 1 ? 's' : '') + ' to go.');
          }
        }
      ...
      case 'zap':
        s.x += s.vx;
        for (const e of G.ents) {
          if (!e.dead && ['rat', 'gull', 'drone', 'robot'].includes(e.type) && aabb({ x: s.x - 6, y: s.y - 3, w: 12, h: 6 }, entBox(e))) {
            damageEnemy(e, 1, s.x); s.t = 0;
          }
        }
...
function updateCart(dt) {
  ...
  p.anim.set('cart_ride'); p.sheet = 'v6'; p.atlas = 'v6a';
  p.anim.update('v6a', dt);
  // pickups still work via updateEnemies
}
...
function updateChaser() {
  ...
  if (c.x >= c.stopX) {
    if (!c.stopped) {
      c.stopped = true;
      toast('ANIMAL CONTROL: "Out of jurisdiction. Ballard bylaws." They salute you.');
      Audio2.tempoMul = 1;
    }
    return;
  }
  ...
  c.x += c.v;
  if (p.x < c.x + 24 && !p.dead) {
    toast('NETTED. "Sir, this is a relocation."');
    killPlayer();
  }
}
...
function bossHitByCan(s) {
  ...
  } else if (b.kind === 'gary' && !b.dead && aabb({ x: s.x - 5, y: s.y - 5, w: 10, h: 10 }, entBox(b))) {
    s.t = 0; bossDamage(1);
  }
}
function bossHitByZap(s) {
  const b = G.boss;
  if (b.kind === 'gary' && !b.dead && b.mode !== 'wait' && aabb({ x: s.x - 6, y: s.y - 3, w: 12, h: 6 }, entBox(b))) {
    s.t = 0;
...
function updateBin(b, p) {
  switch (b.mode) {
    ...
    case 'hop': {
      ...
      if (b.y >= b.homeY) { b.y = b.homeY; b.vy = 0; b.vx = 0; shake(5); Audio2.sfx('stomp');
    ...
    case 'vent':
      ...
      break;
    case 'glitchstart':
      if (b.t === 1) { Audio2.sfx('glitch'); toast('B.I.N.: "DEPLOYING TO PROD ON A FRIDAY."'); }
  ...
  // gravity flip cycling in phase 3
  if (b.glitchT > 0) {
    b.glitchT--;
...
function updateGary(b, p) {
  switch (b.mode) {
    case 'wait': // until mecha picked up (or Jimothy gets pushy)
    ...
    case 'rise':
      ...
      break;
    case 'phone':
      if (b.t === 1) toast('GARY landed. He is CHECKING HIS PHONE. This is your moment.');
      b.y = lerp(b.y, 13 * TILE, 0.1);
      b.x = lerp(b.x, 25 * TILE, 0.05);
      if (b.t > 170) { b.mode = 'rise'; b.t = 0; }
      break;
    case 'dying':
  ...
  if (b.mode !== 'dying' && b.mode !== 'wait' && b.mode !== 'swoop' && !GOD) {
    if (aabb(bodyBox(p, G.gsign), entBox(b))) hurtPlayer(1, b.x);
  }
}

function startExitCutscene() {
  // after bin boss dies — celebrate then finish
